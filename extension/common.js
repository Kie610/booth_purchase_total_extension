...
// 商品1件分の金額は「単価×数量」に、商品行へ1つだけ付くBOOSTを足す。
// 全体の集計は商品の合計ではなく、注文単位の「お支払金額」を正本にする。
// BOOSTの掛け方を誤っていれば、その差額として現れる。
...
// 絞り込み中のCSVにだけ足す列名(「すべて」のCSVは公開契約なので変えない)
...
// 形で保存されている。この末尾の括弧を剥がし、
// 中身をトークンへ分解して、同じトークンを持つ商品どうしをアバターとしてまとめる
// (名簿 avatar-master.js は表記ゆれの統合と表示名のヒントであって辞書ではない)。
//
...
// `__multi__` は既存キーで、意味(複数対応のアバター用アイテム)は変えない
...
// ---- 種別の読み取り(ワールド・ワールド用アイテム・ギミック/ツール) --------
//
