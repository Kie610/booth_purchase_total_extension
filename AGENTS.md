...
4. 現在の要件が求めない抽象化・設定・拡張点を作らない。依頼範囲外の整形・リネームは提案に留める。
...
- 課金、remote作成、権限変更、破壊操作は、対象を読み取りで確認したうえでユーザーの許可を得てから行う。
- 変更を報告するときは、どの観測可能な挙動が変わるかを述べる。
...
- この文書は100行・8192bytes以内、`HANDOFF.agent.md` は原則4096bytes以内に保つ。
  超える詳細は先に `docs/handoff-history.md` へ移す。
