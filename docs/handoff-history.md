...
## 運用ルール合成の再実施(2026-08-16)

bootstrap 提唱の運用ルールを再度当てはめ、2026-08-08 の合成以降に取り込めていなかった差分だけを
`AGENTS.md` へ追記した。既存文書の原文は削除・置換していない。

### 採用(そのまま)

- 依存と先行事例(共通機能を再実装しない、既存手段の確認、先行事例の調査、ライブラリの可否を記憶で断言しない)。
  丸ごと欠落していた。ゼロ依存の現状でも「憶測でライブラリを否定しない」は実害を防ぐため採用。
- 不明・空・失敗・不完全の区別。未取得注文があるときの表示という本製品の中核挙動の根拠が明文化されていなかった。
- 置換成功まで有効データを保持する。バックアップ復元の併合方式とキャッシュ更新の根拠。
- IRREV(課金・remote作成・権限変更・破壊操作)前の明示確認。push と公開のみ既存規約にあり、一般則が無かった。
- 質問を高影響の事項に絞り一度に最大3問とする方針。
- `HANDOFF.agent.md` を引き継ぎ・再開時だけ読む規定。通常セッションのコンテキスト節約に効く。
- 変更報告に観測可能な挙動の変化を含める規定。

### 適合して採用

- なし。追記した項目はいずれも本リポジトリの既存節へそのまま収まった。

### 不採用

- `AGENTS.md` を英語ひな形(`templates/AGENTS.md`)へ全面置換すること。既存の日本語版が機能しており、
  リポジトリ規約で応答・文面を日本語と定めている。提唱に合わせる目的だけの置換は破壊にあたる。
  ひな形の各節に相当する内容(Project contract、Design priorities、Design、Change scope)は
  日本語の該当節として既に存在する。
- 委任プロンプトの7項目と報告項目の全文転記。`subagent-delegation-tiers` スキルと
  グローバル指示で既にカバーされており、`AGENTS.md` の 8192bytes 枠を圧迫する。
  リポジトリ固有の要点(専用worktree・件数無しの合格報告を採用しない)は既存節に残っている。
- `SECURITY.md` の新設。外部送信が無くローカル完結のため、受付は GitHub Issues で足りる。
- `.env.example`。秘密情報を持たない。
- CI の新規導入。全体テストがブラウザ実行前提(`python -m http.server` + `/test/index.html`)であり、
  CI 化は本合成の範囲を超える別作業。
- 5文書構成・handoff v1 schema・プロジェクト契約・設計優先順位・委任と検品。
  2026-08-08 の合成で導入済みのため、今回の変更は無い。

### 検証

- `AGENTS.md`: 93行 / 6851bytes(LF正規化)。上限 100行 / 8192bytes 内。
- `HANDOFF.agent.md`: 3831bytes(LF正規化)。上限 4096bytes 内。
- `CLAUDE.md`: 1行目が `@AGENTS.md` であることを確認。
- `HANDOFF.md` のリンク先5件がすべて存在することを確認。
- `node --check`: extension/ と test/ の 17ファイル、failed=0。
- `git diff --check`: 空白エラーなし。変更は `AGENTS.md` のみ(記録追記前の時点)。
- ブラウザ全体テストと `tools/release.ps1` は未実行。文書のみの変更で JavaScript と配布物に差分が無いため。

## 運用ルール合成の第2回再実施(2026-08-16、6文書構成への移行)

同日の1回目(前節)より後に、`docs/agent-appendix.md` を含む6文書構成とブランチ運用の変更を提唱する
更新版プロンプトを受領し、再度当てはめた。既存文書の規則は1つも削除していない。移動した規則は
すべて `docs/agent-appendix.md` へ写している。

### 採用(そのまま)

- 6文書構成への移行。`docs/agent-appendix.md` を新設し、`AGENTS.md` は「タスクの内容を知る前に有効で
  なければならない規則」だけに絞った。`AGENTS.md` には補遺を読む条件を1行だけ残し、規則の重複を作っていない。
- ひな形の必須4節を欠かさず配置。`Design priorities`(既存)に加え、`Design`・`Change scope` を新設し、
  `Dependencies and prior art`(1回目で追加済み)を維持した。
- `Design` の欠落規則: 端から端まで動く最小構成から段階的に足す / 動いている製品を未完成の作り直しと
  引き換えにしない / モジュールの変更理由を1つに保つ / 暫定実装は内部のみで、永続化データと公開契約では許さない。
- `Change scope` の欠落規則: 到達不能になったコードの削除は範囲内 / 観測可能な挙動の変化を述べる。
- `Invariants` の authoritative source。集計済みの結果を唯一の源とし、UI・CSV出力・共有・バックアップで
  同じ値を使う規則として実値で埋めた。
- `Safety and truth` の「秘密情報と不要な個人情報をコミットしない」。
- `Commands` の「完了を報告する前に該当コマンドをすべて実行する」。
- `Handoff maintenance` の「実質的な進捗・判断・検証・blockerの変化で更新する」「liveな証拠を記録済み
  メタデータより優先し、欠けている値を捏造しない」。
- 委任プロンプトの7項目、検品で裏を取る項目、委任階層、並行実装のworktree運用。1回目は
  `AGENTS.md` の byte 枠を理由に不採用としたが、補遺の新設で収容先ができたため採用した。
- `HANDOFF.md` を4文書(`HANDOFF.agent.md`・`AGENTS.md`・`docs/agent-appendix.md`・`docs/handoff-history.md`)
  への索引へ更新。既存の `docs/improvement-plan.md` と `docs/` への行は本リポジトリ固有の資料として残した。

### 適合して採用

- 節見出し。ひな形は「節見出しは他文書から名指しされるため翻訳しない」と定めるが、本リポジトリは
  既存の日本語見出しで運用しており、履歴からも日本語名で参照している。全面英語化は機能している既存構成の
  リネームにあたるため、日本語見出しを保ったまま英語識別子を併記する形にした
  (例: `## 設計の優先順位 (Design priorities)`)。対象は `Scope and precedence`・`Project contract`・
  `Design priorities`・`Design`・`Dependencies and prior art`・`Change scope` の6節。
- ひな形本文の日本語化。ひな形自体が日本語のため翻訳は発生していない。既存の日本語文面と語調を保ち、
  規則の要約・省略・削除と断定表現の弱化は行っていない。
- `Design priorities` の項目4。既存は「現在の要件が求めない抽象化・設定・拡張点を作らない。依頼範囲外の
  整形・リネームは提案に留める」だった。ひな形の項目4「長期的な構造の一貫性を保つ」へ差し替えたが、
  元の2つの規則は失わせず、前半を `Design`、後半を `Change scope` へ移した。
- IRREV の列挙。既存の「課金、remote作成、権限変更、破壊操作」と「push、公開、配布先への書き込みは
  対象と検証結果を示して許可を得る」を1行へ統合し、ひな形の reset・上書き・削除・merge・deploy・
  可視性変更を加えた上位集合にした。「対象と検証結果を示して」の条件は保持している。

### 不採用(既存優先)

- **ブランチ運用の変更(恒久的な `main` を置かず、バージョン名ブランチと `dev` の2種で運用する)**。
  本リポジトリは `main` を保持し、正式リリース時に検証済みバージョンブランチへ `main` を fast-forward
  同期する運用が機能している。GitHub Release と `v1.0.0`・`v1.1.0` タグもこの前提で公開済み。
  `main` の廃止は公開済みリポジトリの構造変更であり、提唱に合わせる目的だけでの破壊にあたる。
  ユーザーへ質問として提示し、回答until既存規約を維持する。
- 並行ブランチ名を `dev/<topic>` へ変更すること。本リポジトリは `claude/<topic>`・`codex/<topic>` で
  並行実装の実績があり(worktree一覧に9本)、改名の利益がない。
- リリース時に `dev` をバージョン名へリネームする手順。上のブランチ運用を採らないため前提が成立しない。
  既存の「開発用ブランチをバージョンブランチへ統合し、`main` を ff 同期する」手順を維持する。
- `SECURITY.md`・`.env.example`・CI の新規導入。1回目と同じ理由(外部送信なし、秘密情報なし、
  全体テストがブラウザ実行前提)。

### 検証

- `AGENTS.md`: 96行 / 6869bytes(LF正規化)。上限 100行 / 8192bytes 内。
- `docs/agent-appendix.md`: 77行 / 6764bytes。上限の定めなし。
- `HANDOFF.agent.md`: 4028bytes(LF正規化)。上限 4096bytes 内。
- `CLAUDE.md`: 1行目が `@AGENTS.md`。共有規則の重複なし(コメント3行のみ)。
- `HANDOFF.md` のリンク先6件がすべて存在することを確認。
- `git diff` の削除行26件を1件ずつ追跡し、すべてに移動先または統合先があることを確認。
  追跡中に「push許可の条件から『対象と検証結果を示して』が落ちる」欠落を1件検出し、復元した。
- `node --check`: extension/ と test/ の 17ファイル、failed=0。
